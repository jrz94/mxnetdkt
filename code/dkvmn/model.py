...
class ModelDkvmn(object):
    ...
    def sym_gen(self):
        ...
        # (batch_size * seqlen, 50)
        input_embed_content = mx.sym.FullyConnected(data=input_embed_content, num_hidden=50, name="input_embed_content")
        # (batch_size * seqlen, 50)
        input_embed_content = mx.sym.Activation(data=input_embed_content, act_type='tanh',
                                                name="input_embed_content_tanh")
        # (batch_size * seqlen, final_fc_dim)
        read_content_embed = mx.sym.FullyConnected(
            data=mx.sym.Concat(all_read_value_content, input_embed_content, num_args=2, dim=1),
