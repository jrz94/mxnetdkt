...
from model import ModelDkvmnLstm
...
def train_one_dataset(params, file_name, train_q_data, train_qa_data, valid_q_data, valid_qa_data):
    ...
    g_model = ModelDkvmnLstm(n_question=params.n_question,
...
if __name__ == '__main__':
    ...
    if not params.test:
        ...
        log.info("\n")
        log.info("train_q_data.shape " + str(train_q_data.shape) + ' ' + str(
            train_q_data))  ###(3633, 200) = (#sample, seqlen)
        log.info("train_qa_data.shape " + str(train_qa_data.shape) + ' ' + str(
            train_qa_data))  ###(3633, 200) = (#sample, seqlen)
        log.info("valid_q_data.shape " + str(valid_q_data.shape))  ###(1566, 200)
